<div class="wrap">
    ...
    <?php if(isset( $_GET['settings-updated'])) {
        ...
        if($gridder_apply == "gridder_apply_all"){
            GridderOptions::applyDesktopDefaultsToAllExistingGrids(true);
        }
        else if($gridder_apply == "gridder_apply_all_but_row"){
            GridderOptions::applyDesktopDefaultsToAllExistingGrids(false);
        }

        $phone_gridder_apply = get_option('phone_gridder_apply', '');
        if($phone_gridder_apply == "phone_gridder_apply_all"){
            GridderOptions::applyPhoneDefaultsToAllExistingGrids(true);
        }
        else if($phone_gridder_apply == "phone_gridder_apply_all_but_row"){
            GridderOptions::applyPhoneDefaultsToAllExistingGrids(false);
        }
    ...
    <div class="lay-explanation <?php echo get_option('expand-how_to_use_gridder_defaults', 'expanded'); ?>" data-expand-status-option-name="expand-how_to_use_gridder_defaults">
        ...
        <div class="lay-explanation-inner">
            ...
            <?php
                $misc_options_extra_gridder_for_phone = get_option('misc_options_extra_gridder_for_phone', 'on');
                if($misc_options_extra_gridder_for_phone == "on"){
                    ?>
                    <p>The Custom Phone Layout Gridder Defaults only apply to custom phone layouts created with the <span style="border-radius:2px;border: 1px solid #ccc;color:#333;background-color:#fff;display:inline;padding:4px 10px;"><span class="dashicons dashicons-smartphone"></span></span> phone button in the Gridder. For automatically generated phone layouts please set the values in "Customize" &rarr; "Mobile" &rarr; "Mobile Spaces".</p>
                    <?php
                }
            ?>
        </div>
